...
# 2. for each transcript - find the sequence according to the GTF file
# 3. get the ORF of the sequence
# 4. look for the target exon within the sequence
# 5. Count the exons that has been found and the exons that hasent.
# 6. Add "Exon was found?" column to the table
import csv, requests, multiprocessing, sys, os, subprocess, re, time
import pandas as pd
...
def getSeqFromGTF (transcript_id, chr):
  filtered_gtf_df = gtf_df[(gtf_df['seqname'] == chr) & (gtf_df['feature'] == 'exon')]
  transcript_found = False
  transcript_seq = ""
  for index, row in filtered_gtf_df.iterrows():
    if row['attribute']['transcript_id'] == transcript_id and not transcript_found:
      transcript_found = True
      print(f"Transcript {transcript_id} was found in the GTF file.")
    if row['attribute']['transcript_id'] == transcript_id and transcript_found:
...
def getCDSFromGTF (transcript_id, chr):
  ...
  filtered_gtf_df = gtf_df[(gtf_df['seqname'] == chr) &
                           ((gtf_df['feature'] == 'CDS') | (gtf_df['feature'] == 'stop_codon'))]
  if filtered_gtf_df.shape[0] == 0:
    print(f"No CDS for transcript {transcript_id}. Skipping.")
    return cds_seq
  for index, row in filtered_gtf_df.iterrows():
    if row['feature'] == 'CDS' and row['attribute']['transcript_id'] == transcript_id and not cds_found:
      cds_found = True
      print(f"CDS of {transcript_id} was found in the GTF file.")
    if row['feature'] == 'CDS' and cds_found:
      chr, start, end, strand = row['seqname'], row['start'], row['end'], row['strand']
      ...
      continue
    if row['feature'] == 'stop_codon' and cds_found:
      chr, start, end, strand = row['seqname'], row['start'], row['end'], row['strand']
...
def run_with_GTF(index,row):
  ...
  # get transcript seq
  if transcript_id in transcripts_dict.keys():
  ...
  # check if exon in the transcript
  ...
  if transcript_id in cds_dict.keys():
...
gene_map = pd.read_csv(gtf_map, delimiter='\t', index_col=False)
...
merged_results = pd.merge(results, gene_map, on='Gene.Symbol', how='inner')
merged_results = merged_results.rename(columns={merged_results.columns[8]: 'dPSI'})
